...
public sealed class WorkspaceOrchestratorTests
{
    ...
    public void CreateWorkspace_WritesCanonicalAndGeneratedFiles()
    {
        ...
        try
        {
            ...
            Assert.Contains("bash /opt/opencode-workspace/config/workspace-init.sh", File.ReadAllText(snapshot.Paths.ProvisionScriptPath));
            Assert.True(File.Exists(Path.Combine(snapshot.Paths.ConfigPath, "workspace-image-tooling.sh")));
            Assert.True(File.Exists(Path.Combine(snapshot.Paths.ConfigPath, "workspace-image-tooling.common-development-tooling.sh")));
            Assert.Contains("/home/opencode/.local/share/opencode/log", File.ReadAllText(Path.Combine(snapshot.Paths.ConfigPath, "workspace-init.sh")));
    ...
    public void CreateWorkspace_ForPublishingTex_WritesExamplesValidationAndOnboarding()
    {
        ...
        try
        {
            ...
            var agents = File.ReadAllText(Path.Combine(snapshot.Paths.RootPath, "AGENTS.md"));
            var toolingProvision = File.ReadAllText(Path.Combine(snapshot.Paths.ConfigPath, "workspace-image-tooling.sh"));
            ...
            Assert.Contains("command -v typst", toolingProvision);
            Assert.Contains("install /tmp/typst-install/typst-*/typst /usr/local/bin/typst", toolingProvision);
        }
        finally
        {
            DeleteTempRoot(tempRoot);
    ...
    public async Task ProvisionAsync_WhenProvisioningFails_DoesNotMarkRuntimeStateAsProvisioned()
    {
        ...
        try
        {
            ...
            await Assert.ThrowsAsync<WorkspaceProvisioningException>(() => orchestrator.ProvisionAsync(snapshot));
    ...
    private sealed class StubContainerRuntime : IContainerRuntime
    {
        ...
        {
            var userExists = _provisioned || ProvisionCallCount > 0 || UserExistsInitially;
        ...
        public Task<ProcessResult> RunSimpleDockerCommandAsync(IEnumerable<string> arguments, Action<CommandLogEntry>? log = null, CancellationToken cancellationToken = default)
        {
            ...
            if (argumentList.Count >= 4 && argumentList[0] == "exec" && string.Equals(argumentList[2], "bash", StringComparison.Ordinal) && argumentList[3].StartsWith("/opt/opencode-workspace/config/", StringComparison.Ordinal))
            {
                var scriptPath = argumentList[3];
                if (scriptPath.EndsWith("workspace-init.sh", StringComparison.Ordinal))
                {
                    ProvisionCallCount++;
                    _provisioned = true;
                    return Task.FromResult(ProvisionScriptResultFactory?.Invoke() ?? Success($"docker exec {argumentList[3]}"));
                }

                return Task.FromResult(Success($"docker exec {argumentList[3]}"));
            }
