...
public sealed class GeneratedArtifactsTests
{
    ...
    public void ProvisioningGenerator_IncludesGeneratedHeaderAndOpenCodeInstall()
    {
        ...
        Assert.Contains("opencode_apt_install git curl", toolingScript);
        Assert.Contains("cat > /etc/apt/apt.conf.d/99opencode-image-build <<'EOF'", toolingScript);
        Assert.Contains("Acquire::Retries \"5\";", toolingScript);
        Assert.Contains("opencode_apt_install() { apt-get install -y \"$@\"; }", toolingScript);
        Assert.Contains("opencode_ensure_dir_owned_if_user_exists()", toolingScript);
        Assert.Contains("opencode_run_as_opencode_or_root()", toolingScript);
        Assert.Contains("opencode_apt_cleanup() { rm -rf /var/lib/apt/lists/*; }", toolingScript);
    ...
    public void ProvisioningGenerator_ForDocumentationWorkspace_IncludesToolingAndFontSetup()
    {
        var script = new WorkspaceImageToolingScriptGenerator().Generate(new ResolvedWorkspace
        {
            ...
            {
                "command -v typst >/dev/null 2>&1 || opencode_apt_install_optional typst",
                "playwright_browsers_path=/ms-playwright",
                "opencode_ensure_dir_owned_if_user_exists \"${playwright_browsers_path}\"",
                "opencode_run_as_opencode_or_root 'export PLAYWRIGHT_BROWSERS_PATH=/ms-playwright; playwright install chromium'",
                "playwright_global_root=$(npm root -g)",
                "export PLAYWRIGHT_BROWSERS_PATH=/ms-playwright",
                "playwright_chromium_path=$(opencode_run_as_opencode_or_root 'export PLAYWRIGHT_BROWSERS_PATH=/ms-playwright; export NODE_PATH=$(npm root -g); node -e \"const { chromium } = require(process.env.NODE_PATH + \\\"/playwright\\\"); process.stdout.write(chromium.executablePath());\"')",
                "echo \"[document-processing] Resolved Playwright Chromium executable: ${playwright_chromium_path}\"",
                "fc-cache -fv",
            },
        });

        Assert.Contains("opencode_apt_install_optional() { apt-get install -y --no-install-recommends \"$@\"; }", script);
        Assert.Contains("opencode_apt_install pandoc fonts-crosextra-carlito fonts-jetbrains-mono", script);
        ...
        Assert.Contains("opencode_apt_install_optional typst", script);
        Assert.Contains("playwright install chromium", script);
        Assert.Contains("playwright_browsers_path=/ms-playwright", script);
        Assert.Contains("opencode_ensure_dir_owned_if_user_exists \"${playwright_browsers_path}\"", script);
        Assert.Contains("opencode_run_as_opencode_or_root 'export PLAYWRIGHT_BROWSERS_PATH=/ms-playwright; playwright install chromium'", script);
        Assert.Contains("export PLAYWRIGHT_BROWSERS_PATH=/ms-playwright", script);
        Assert.Contains("opencode_run_as_opencode_or_root 'export PLAYWRIGHT_BROWSERS_PATH=/ms-playwright; export NODE_PATH=$(npm root -g)", script);
        Assert.Contains("Resolved Playwright Chromium executable", script);
        Assert.Contains("fc-cache -fv", script);
    }

    [Fact]
    public void WorkspaceImageToolingScripts_DoNotModifyDockerManagedHostFiles()
    {
        var provider = new BuiltInCatalogProvider(TestPaths.CatalogRoot);
        var resolver = new WorkspaceResolver(provider.LoadFeatures(), provider.LoadServices(), provider.LoadCapabilities(), provider.LoadKnowledgePacks());
        ...
        foreach (var template in provider.LoadTemplates())
        {
            var resolved = resolver.Resolve(expander.Expand($"smoke-{template.Id}", template));
            var layout = generator.GenerateLayout(resolved);
            Assert.NotEmpty(layout.LayerScripts);

            foreach (var layer in layout.LayerScripts)
            {
                foreach (var fragment in forbiddenFragments)
                {
                    Assert.DoesNotContain(fragment, layer.Content, StringComparison.Ordinal);
                }
            }
        }
    }
