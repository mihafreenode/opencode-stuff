    ...
    private static WorkspaceProviderHealthSnapshot? BuildSynchronizationProvider(WorkspaceSnapshot snapshot, DateTimeOffset timestamp)
    {
        ...
        return new WorkspaceProviderHealthSnapshot
        {
            ...
            [
                ...
                new WorkspaceHealthFact { Label = "workspace", Value = environment?.WorkspaceName ?? string.Empty },
                new WorkspaceHealthFact { Label = "schema", Value = environment?.ParsingSchema ?? string.Empty },
                new WorkspaceHealthFact { Label = "application", Value = environment?.ApplicationId?.ToString() ?? string.Empty },
                ...
                new WorkspaceHealthFact { Label = "last validation", Value = environment?.LastValidationUtc?.ToString("u") ?? "Never" },
                new WorkspaceHealthFact { Label = "last export", Value = environment?.LastExportUtc?.ToString("u") ?? "Never" },
                new WorkspaceHealthFact { Label = "last pull", Value = environment?.LastPullUtc?.ToString("u") ?? "Never" },
