...
public sealed class OracleWorkspaceProvisioningScriptGenerator
{
    public string Generate(ResolvedWorkspace workspace)
    {
        ...
        builder.AppendLine("ensure_demo_user_ready() { local setup_script=/tmp/oracle-demo-user-setup.sql; sqlplus -L -S \"${oracle_sysdba_connection}\" @\"${setup_script}\" >/tmp/oracle-demo-user-setup.out 2>&1 || { cat /tmp/oracle-demo-user-setup.out >&2; oracle_fail \"Oracle administrator password does not match the running database.\" \"$(tr '\\n' ' ' < /tmp/oracle-demo-user-setup.out | xargs || true)\" \"Rebuild Runtime.\"; }; cat /tmp/oracle-demo-user-setup.out; }");
        builder.AppendLine("query_database_open_mode() { sqlplus -S \"${oracle_sysdba_connection}\" <<'SQL'\nSET PAGESIZE 0\nSET FEEDBACK OFF\nSET HEADING OFF\nSET VERIFY OFF\nSELECT open_mode FROM v\\$database;\nEXIT\nSQL\n}");
        builder.AppendLine("query_pdb_open_mode() { sqlplus -S \"${oracle_sysdba_connection}\" <<SQL\nSET PAGESIZE 0\nSET FEEDBACK OFF\nSET HEADING OFF\nSET VERIFY OFF\nSELECT open_mode FROM v\\$pdbs WHERE name = UPPER('${ORACLE_SERVICE_NAME}');\nEXIT\nSQL\n}");
        builder.AppendLine("query_xdb_status() { sqlplus -S \"${oracle_sysdba_connection}\" <<'SQL'\nSET PAGESIZE 0\nSET FEEDBACK OFF\nSET HEADING OFF\nSET VERIFY OFF\nSELECT status FROM dba_registry WHERE comp_id = 'XDB';\nEXIT\nSQL\n}");
        ...
        if (hasOracleApex)
        {
            ...
            builder.AppendLine("query_apex_registry() { sqlplus -S \"${oracle_sysdba_connection}\" <<'SQL'\nSET PAGESIZE 0\nSET FEEDBACK OFF\nSET HEADING OFF\nSET VERIFY OFF\nSELECT comp_name || '|' || version || '|' || status FROM dba_registry WHERE comp_id = 'APEX';\nEXIT\nSQL\n}");
            builder.AppendLine("query_apex_schema_count() { sqlplus -S \"${oracle_sysdba_connection}\" <<'SQL'\nSET PAGESIZE 0\nSET FEEDBACK OFF\nSET HEADING OFF\nSET VERIFY OFF\nSELECT COUNT(*) FROM dba_users WHERE username LIKE 'APEX_%';\nEXIT\nSQL\n}");
            builder.AppendLine("query_ords_public_user_count() { sqlplus -S \"${oracle_sysdba_connection}\" <<SQL\nSET PAGESIZE 0\nSET FEEDBACK OFF\nSET HEADING OFF\nSET VERIFY OFF\nSELECT COUNT(*) FROM dba_users WHERE username = UPPER('${ORACLE_ORDS_PUBLIC_USER}');\nEXIT\nSQL\n}");
            builder.AppendLine("query_ords_metadata_count() { sqlplus -S \"${oracle_sysdba_connection}\" <<'SQL'\nSET PAGESIZE 0\nSET FEEDBACK OFF\nSET HEADING OFF\nSET VERIFY OFF\nSELECT COUNT(*) FROM dba_users WHERE username = 'ORDS_METADATA';\nEXIT\nSQL\n}");
