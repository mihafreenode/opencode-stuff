...
public sealed class OracleWorkspaceProvisioningScriptGenerator
{
    public string Generate(ResolvedWorkspace workspace)
    {
        ...
        if (hasOracleApex)
        {
            ...
            builder.AppendLine($"oracle_apex_recommended_database_image={QuoteShell(OracleDatabaseImageCatalog.DefaultDatabaseImage)}");
            ...
            builder.AppendLine($"oracle_known_bad_apex_image() {{ [ \"${{oracle_database_image}}\" = {QuoteShell(OracleDatabaseImageCatalog.KnownIncompatibleApexDatabaseImage)} ]; }}");
            builder.AppendLine("ensure_apex_xdb_registry_valid() { oracle_capture_xdb_diagnostics; local root_status pdb_status invalid_object_count representative_errors root_functional_probe pdb_functional_probe recommendation; root_status=$(oracle_extract_registry_status \"${oracle_root_registry}\"); pdb_status=$(oracle_extract_registry_status \"${oracle_pdb_registry}\"); if [ \"${root_status}\" = 'VALID' ] && [ \"${pdb_status}\" = 'VALID' ]; then return 0; fi; invalid_object_count=$(( $(oracle_count_lines \"${oracle_root_invalid_objects}\") + $(oracle_count_lines \"${oracle_pdb_invalid_objects}\") )); root_functional_probe=$(query_xdb_functional_probe_in_container \"${oracle_root_container}\" | oracle_compact_output || true); pdb_functional_probe=$(query_xdb_functional_probe_in_container \"${oracle_target_container}\" | oracle_compact_output || true); representative_errors=$(oracle_first_lines \"${oracle_root_errors}\"); if [ -z \"${representative_errors}\" ]; then representative_errors=$(oracle_first_lines \"${oracle_pdb_errors}\"); fi; if oracle_known_bad_apex_image; then recommendation=\"Use Oracle image ${oracle_apex_recommended_database_image} for APEX workspaces and reset the runtime after changing the image.\"; else recommendation='Use an Oracle image/version that reports XDB as VALID before running the Oracle APEX installer.'; fi; oracle_fail 'Oracle APEX prerequisite validation failed.' \"database_image=${oracle_database_image}; database_version=${oracle_database_version:-unknown}; root_xdb_registry=${oracle_root_registry:-missing}; pdb_xdb_registry=${oracle_pdb_registry:-missing}; invalid_object_count=${invalid_object_count}; dba_errors=${representative_errors:-none}; root_functional_probe=${root_functional_probe:-failed}; pdb_functional_probe=${pdb_functional_probe:-failed}\" \"${recommendation}\"; }");
