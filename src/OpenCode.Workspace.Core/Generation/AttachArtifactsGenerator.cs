...
public sealed class AttachArtifactsGenerator
{
    public string GenerateWindowsTerminalWrapper(WorkspaceDefinition definition, WorkspacePaths paths, GeneratedArtifactRuntimeMetadata? runtimeMetadata = null)
    {
        ...
        builder.AppendLine("    $pathCheck = Invoke-DockerCheck -Arguments @('exec', '--user', $attachUser, '-w', $workspaceDirectory, $containerName, 'bash', '-lc', 'printf \"PATH=%s\\n\" \"$PATH\"; npm_prefix=\"\"; npm_root=\"\"; if command -v npm >/dev/null 2>&1; then npm_prefix=$(npm prefix -g 2>/dev/null || printf \"\"); npm_root=$(npm root -g 2>/dev/null || printf \"\"); printf \"npm prefix -g: %s\\n\" \"${npm_prefix:-unavailable}\"; printf \"npm root -g: %s\\n\" \"${npm_root:-unavailable}\"; fi; for candidate in /usr/bin/opencode /usr/local/bin/opencode; do if [ -e \"$candidate\" ]; then printf \"candidate: %s\\n\" \"$candidate\"; ls -l \"$candidate\"; fi; done; if [ -n \"${npm_prefix}\" ] && [ -e \"${npm_prefix}/bin/opencode\" ]; then printf \"candidate: %s\\n\" \"${npm_prefix}/bin/opencode\"; ls -l \"${npm_prefix}/bin/opencode\"; fi; if [ -n \"${npm_root}\" ] && [ -e \"${npm_root}/.bin/opencode\" ]; then printf \"candidate: %s\\n\" \"${npm_root}/.bin/opencode\"; ls -l \"${npm_root}/.bin/opencode\"; fi')");
