...
public sealed class ProvisioningScriptGenerator
{
    ...
    public string Generate(ResolvedWorkspace workspace, GeneratedArtifactRuntimeMetadata? runtimeMetadata = null)
    {
        ...
        if (isOracleDemoWorkspace)
        {
            ...
            builder.AppendLine("  setup_output=$(sqlplus -L -S \"${sysdba_connection}\" @\"${oracle_demo_user_setup_script}\" 2>&1) || {");
            ...
            builder.AppendLine("run_with_timeout 120 'Oracle demo user setup' 'Rebuild Runtime if the Oracle data volume contains stale credentials.' ensure_demo_user_ready");
            ...
            if (hasOracleApex)
            {
                ...
                builder.AppendLine("  fi");
                builder.AppendLine("  wait_for_command 180 10 'ORDS readiness' 'Check the oracle-ords container logs and recreate the ORDS container if configuration did not complete.' bash -lc \"curl -fsSI '${oracle_ords_landing_url}' >/dev/null\"");
                builder.AppendLine("}");
                builder.AppendLine("verify_apex_login_route() {");
                builder.AppendLine("  if curl -fsSI \"${oracle_apex_url}\" >/dev/null 2>&1; then");
                builder.AppendLine("    return 0");
                builder.AppendLine("  fi");
                builder.AppendLine("  if curl -fsSI \"${oracle_ords_landing_url}\" >/dev/null 2>&1; then");
                ...
                builder.AppendLine("    return 0");
                builder.AppendLine("  fi");
                builder.AppendLine("  wait_for_command 180 10 'APEX login readiness' 'Check ORDS configuration and confirm APEX installed successfully before retrying.' bash -lc \"curl -fsSI '${oracle_ords_landing_url}' >/dev/null\"");
                builder.AppendLine("}");
