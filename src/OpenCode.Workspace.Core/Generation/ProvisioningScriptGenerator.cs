    ...
    public string Generate(ResolvedWorkspace workspace, GeneratedArtifactRuntimeMetadata? runtimeMetadata = null)
    {
        ...
        if (isOracleDemoWorkspace)
        {
            ...
            if (hasOracleApex)
            {
                ...
                builder.AppendLine("  fi");
                builder.AppendLine("  if [ \"${attempt}\" -eq 6 ]; then");
                builder.AppendLine("    oracle_fail \"Oracle APEX login route is not reachable.\" \"APEX login URL ${oracle_apex_url} did not respond after 6 checks\" \"Check ORDS configuration and confirm APEX installed successfully before retrying.\" medium");
                builder.AppendLine("  fi");
