...
public sealed class ProvisioningScriptGenerator
{
    ...
    public string Generate(ResolvedWorkspace workspace)
    {
        ...
        builder.AppendLine("  set -a");
        builder.AppendLine("  . /workspace/.env");
        builder.AppendLine("  set +a");
        ...
        if (isOracleDemoWorkspace)
        {
            ...
            if (hasOracleApex)
            {
                ...
                builder.AppendLine("SQL");
                builder.AppendLine("  ) || true");
                builder.AppendLine("  configure_apex_cdn || true");
                builder.AppendLine("  sqlplus -S \"sys/change-on-first-demo@//oracle-demo:1521/FREEPDB1 as sysdba\" <<'SQL'");
                builder.AppendLine("BEGIN");
                ...
                builder.AppendLine("  fi");
                builder.AppendLine("  if [ \"${attempt}\" -eq 6 ]; then");
                builder.AppendLine("    echo \"OracleRuntimeFailure: APEX login route not reachable\" >&2");
                builder.AppendLine("    exit 1");
                builder.AppendLine("  fi");
