...
For implementation status, prerequisites, and experimental boundaries, start with [Oracle and Oracle APEX Integration](integrations/oracle-apex.md).
...
The generated workspace ships the sample schema and an APEX source placeholder. Building a complete `Customer Orders Demo` is a tutorial goal, not a completed application generated by the template. Use the shared sample domain to build it around:
